...
resource "routeros_ip_pool" "ip4_pool_infra" {
  name   = "pool_infra"
  ranges = ["10.0.30.100-10.0.30.250"]
}

resource "routeros_ip_pool" "ip4_pool_mgmt" {
  name   = "pool_mgmt"
  ranges = ["10.0.99.100-10.0.99.250"]
}
...
resource "routeros_ip_dhcp_server_network" "ip4_dhcp_servernet_infra" {
  address    = "10.0.30.0/24"
  gateway    = "10.0.30.1"
  dns_server = local.dns_servers
}
resource "routeros_ip_dhcp_server" "ip4_dhcp_server_infra" {
  address_pool = routeros_ip_pool.ip4_pool_infra.name
  interface    = routeros_interface_vlan.interface_vlan_infra.name
  name         = "ip4_dhcp_infra"
}

resource "routeros_ip_dhcp_server_network" "ip4_dhcp_servernet_mgmt" {
  address    = "10.0.99.0/24"
  gateway    = "10.0.99.1"
  dns_server = local.dns_servers
}
resource "routeros_ip_dhcp_server" "ip4_dhcp_server_mgmt" {
  address_pool = routeros_ip_pool.ip4_pool_mgmt.name
  interface    = routeros_interface_vlan.interface_vlan_mgmt.name
  name         = "ip4_dhcp_mgmt"
}
